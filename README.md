...
- [Wall-display on-screen keyboard](docs/touch-keyboard.md)
...
The primary Home, Calendar, Tasks, Chores, and Rewards experience now runs as one cohesive routed workspace with a persistent household shell, pointer and touch gestures, browser-history-safe URLs, reduced-motion support, matching feature surfaces, and keyboard-visible control focus. An app-native on-screen keyboard is available for keyboardless wall displays, with automatic shared-display detection and an explicit Auto/On/Off device preference. Dashboard Personalization and Weather Increment 1 is deployed and staging verified with household-specific greetings, authenticated private photo variants and focal positioning, approximate weather location, NWS current conditions, and an accessible forecast dialog. Dashboard Personalization Increment 2 extends the same private, authenticated photo treatment to household-member profiles and attribution surfaces and has been owner-verified in staging. The public demonstration photo remains only a fallback; household and member uploads are private and backend-authorized.

The full Calendar workspace now uses a household-local month grid on wall displays, tablets, and desktops, with events placed on their actual dates and a detailed selected-day agenda. Phones receive an explicit date-grouped agenda. Month navigation, calendar overflow, all-day and cross-midnight placement, household locale/time-zone/week-start preferences, and selected-date event creation remain frontend presentation concerns; Google Calendar is still the event source of truth.
