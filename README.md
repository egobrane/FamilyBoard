...
Health, Google-login initiation, and invitation prepare/inspection endpoints are anonymously usable; invitation preparation additionally requires JSON from the exact configured frontend origin. Account identity, household setup, selected-household context, household heading, configured calendar events, chores, points, and rewards come from the authenticated API. Identity and Household Management Increments 1–6, Google Calendar Increments 1–3, Chore Management Increments 1–3, and Reward Management Increment 1 are deployed and staging verified with the matching Netlify frontend. Calendar event management remains deliberately limited to simple, non-recurring events created through Family Dashboard; externally created and recurring events are read-only. Approved chores atomically award snapshotted points, balances derive from immutable transactions, and corrections use compensating entries. Reward requests reserve points immediately and release them through immutable compensating entries on rejection or cancellation. Advanced inventory, reward images, notifications, recurring Calendar mutation, Tasks, weather, and photo storage remain deferred. OAuth codes and tokens, PINs, PIN hashes, and peppers never reach the frontend.
