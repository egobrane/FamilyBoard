...
Health, Google-login initiation, and invitation prepare/inspection endpoints are anonymously usable; invitation preparation additionally requires JSON from the exact configured frontend origin. Account identity, household setup, selected-household context, household heading, and configured calendar events come from the authenticated API; chore, reward, weather, and photo content remains placeholder data. Identity and Household Management Increments 1–6 and Google Calendar Increment 1 are deployed and verified in Azure staging with the matching Netlify frontend. Controlled Calendar event creation is implemented behind a separate disabled-by-default gate and still requires migration, Google consent-scope activation, deployment, and staging verification. Google remains the event source of truth; event editing/deletion, Tasks, chores, rewards, weather, and photo storage remain deferred. OAuth codes and tokens, PINs, PIN hashes, and peppers never reach the frontend.
