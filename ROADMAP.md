...
- The staging API runs reviewed digest `sha256:93ac0319cc0f870c2d12059dd343bbeae7b5b069765342258ae0430570d2dfcf` on healthy revision `family-dashboard-staging-api--0000032` with 100% traffic. Netlify production deploy `6a8f3db6556aee000886b5b1` serves merged operational-hardening commit `04e1a12672950bd88467c85b0436b26673530ff4`; its unchanged Version 1 application bundle is `/assets/index-D2ZJAxw2.js`.
...
- PWA freshness hardening is deployed: update discovery occurs after React mounts, checks repeat on focus/visibility and long-running displays, forms block activation, safe idle displays may update, and Netlify separates revalidated shell assets from immutable hashed assets. Cloudflare now bypasses edge caching for only `family.egobrane.net/sw.js`, which returns the origin `no-cache, max-age=0, must-revalidate` policy with `cf-cache-status: DYNAMIC`. A safe real Deploy Preview has also been proven without production API access; the two-version physical-device proof remains at its recorded Version 1 checkpoint.
...
- Google Tasks Increment 1 is implemented behind a disabled-by-default gate: authorization is separate from sign-in and Calendar, provider tokens remain encrypted backend-only, household list selections are persisted without task copies, routine reads use disposable caching, and the touch-first interfaces form the genuine Version 2 PWA release candidate.
...
- Configure the dedicated Google Tasks OAuth client and Key Vault secret, deploy/migrate with the gate disabled, activate the reviewed configuration, and complete live consent, selection, revocation, multi-household, shared-display, and leakage checks.
- Use the genuine Google Tasks frontend release as Version 2 for the Safari and wall-display PWA proof. Verify prompt visibility, form protection, safe idle activation, offline retention, session continuity, and the new bundle without manual service-worker cleanup.
