...
- The staging API runs reviewed digest `sha256:909e2e98a4cde3e9fce3d7220b26d4b4d0e4edc80e434e4a2b19befbabcad4c0` on healthy revision `family-dashboard-staging-api--0000021` with both Calendar feature flags enabled. The matching Netlify production PWA is deployed.
...
- Automatic immutable-image handoff is implemented locally: backend publication emits its exact digest to a reusable protected deployment, verifies matching CI/current `main`, migrates first, and no longer requires a digest commit or second push.
- Chore Management Increment 2 is implemented and locally verified: daily, selected-weekday, weekly, and interval schedules generate snapshot assignments through an idempotent PostgreSQL-locked command, with responsive administration UI and portable Compose, K3s, and Azure scheduled-job definitions.
...
- Prove the one-push backend release on GitHub: approve the staging environment once, confirm the same publication digest migrates and deploys, and verify that no repository digest update is created.
- Apply the additive recurring-schedule migration, provision the prefixed Azure chore-generator job, deploy the matching Netlify frontend, and complete the Increment 2 owner staging checklist.
