...
- The staging API runs reviewed multi-architecture digest `sha256:99c704484334a863addfce2f155ee7522e6d8591b7e3cae4e38c24edab168580` on healthy revision `family-dashboard-staging-api--0000037` with 100% traffic. Netlify production deploy `6a8f566b4b7ff400082e1689` serves Tasks implementation commit `52debdf32f2651aac19dcff40253749ed9e87dbc`; its Version 2 application bundle is `/assets/index-Ccml6tL4.js` and compiles the approved `https://api.egobrane.net` origin.
...
- Google Tasks Increment 1 is deployed and activated behind its separate OAuth boundary. The dedicated read-only client, exact callback, Key Vault-only client-secret reference, additive persistence, provider-owned task reads, disposable cache, household source selection, and Version 2 interface are present; live connection, list, revocation, multi-household, shared-display, and leakage checks remain to be owner-exercised before the increment is called staging verified.
- Google Tasks Increment 2 is implemented behind `GoogleTasks__MutationsEnabled=false`: incremental broad-scope authorization, one isolated writable list, top-level creation, completion/reopening, shared-display member attribution, protected ETag versions, and append-only idempotency receipts are ready for migration and staging activation. No task content is stored locally.
...
- Complete live Google Tasks connection, task-list selection, dashboard/full-list, provider-change/cache, revocation/reconnection, disconnect, multi-household isolation, locked shared-display, parent-elevation, responsive-device, and sensitive-data leakage checks.
- Deploy `AddGoogleTaskMutations` with the mutation gate disabled, approve the broad Google Tasks scope, activate the same reviewed digest, reauthorize explicitly, choose one writable list, and exercise private/shared creation plus completion/reopening and retry/conflict boundaries.
