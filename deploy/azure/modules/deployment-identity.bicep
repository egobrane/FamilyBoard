...
param choreGeneratorJobName string
...
resource choreGeneratorJob 'Microsoft.App/jobs@2025-01-01' existing = {
  name: choreGeneratorJobName
}
...
resource choreGeneratorDeploymentRole 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(choreGeneratorJob.id, identity.id, containerAppsJobsContributorRoleId)
  scope: choreGeneratorJob
  properties: {
    roleDefinitionId: containerAppsJobsContributorRoleId
    principalId: identity.properties.principalId
    principalType: 'ServicePrincipal'
