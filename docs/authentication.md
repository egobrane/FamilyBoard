...
4. Publish the reviewed backend commit, record its immutable GHCR digest in `deploy/azure/staging.bicepparam`, commit that parameter-only update, and run the protected `Deploy backend to Azure staging` workflow. Parameter-only commits do not publish another backend image. The migration job must succeed before the reconciled API update.
