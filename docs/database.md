...
- `HouseholdDashboardAppearance` stores optional greeting copy, photo focal coordinates, and the active private-photo asset ID. `HouseholdPhotoAsset` stores only household-scoped storage metadata and attribution, never original image metadata or public URLs. `HouseholdWeatherConfiguration` stores rounded household coordinates, a label, and unit preference; forecast content is never persisted.
