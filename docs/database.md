...
- Chore assignments snapshot an explicit `Assigned` or `Open` mode. Assigned work references an active household member immediately; open work keeps that relationship null until one active member atomically claims it. Claim time, member, request ID, and shared-display context are retained alongside the creator, original household-local due date/time and IANA zone, derived UTC instant, status, skip metadata, and concurrency version.
...
- Chore schedules belong to one household, definition, creating adult profile, and either an active assigned member or an explicit open mode. They retain recurrence, local due-time intent, lifecycle state, next occurrence, and optimistic concurrency. Generated assignments link back to the schedule and use a unique household/schedule/local-date occurrence key while preserving their own assignment-mode snapshots.
...
Migration `AddOpenChoreAssignments` is additive and backfills every existing schedule and assignment to `Assigned`. It makes the member relationship nullable only for explicitly open work, adds database assignment-mode/member constraints, and adds household-scoped unique claim-request and claimant relationships. Existing assigned history is unchanged. Application rollback is safe before open rows are created; after open rows exist, use a forward application fix because an older application does not understand nullable assignees.
