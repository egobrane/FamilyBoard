...
Migration `AddGoogleTaskMutations` is additive. It adds write-target metadata and partial unique indexes enforcing one writable source per household and preventing the same provider list from being writable in multiple households. It creates `GoogleTaskMutationReceipts` with composite household foreign keys and no task-content columns. The feature remains disabled until the migration, reviewed image, OAuth consent scope, and runtime flag are deployed.
