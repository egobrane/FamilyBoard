...
Google Tasks Increment 2 adds `AddGoogleTaskMutations` and the reviewed `GoogleTasks__MutationsEnabled` allowlist setting. The protected workflow captures, applies, verifies, and restores that setting with the immutable image just like the existing integration flags. Deploy with the flag false, migrate first, add the broad Tasks scope to the approved Google consent configuration, then enable the same digest. Disabling the flag rolls back application mutations; revoking the broad provider grant requires disconnecting the Tasks connection and reconnecting read-only.
