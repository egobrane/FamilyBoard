...
The initial infrastructure deployment remains manual under the developer identity. Routine backend deployments use the protected GitHub `staging` environment and OIDC and read the immutable image plus an explicit allowlist of public runtime settings from reviewed `staging.bicepparam`. The workflow compiles that file using a disposable validation-only PostgreSQL placeholder; it never receives or retrieves the real administrator password and does not deploy the complete Bicep template. It runs migrations first, reconciles only the named API and migration job, verifies the ready revision, digest, runtime settings, 100% traffic, and both health endpoints, and attempts to restore the previous API image/settings if deployment verification fails. Database migrations remain forward-only.

This separation protects unrelated resources in `ryan-dev` and preserves the narrow existing OIDC roles. Structural infrastructure, secret-reference, networking, PostgreSQL, identity, or Key Vault changes still require an explicitly reviewed manual Bicep deployment. At the 2026-08-21 checkpoint the live API used the Increment 2 digest while the parameter file named Increment 1; the reconciliation implementation updates the reviewed parameter to the live digest and prevents the next routine release from repeating that drift.
