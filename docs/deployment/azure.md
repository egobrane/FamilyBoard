...
Calendar Increment 2 adds no Azure resource or secret. `enableGoogleCalendarEventCreation` maps to a separate backend feature gate and defaults to `false`, including in staging parameters. Deploy and migrate the reviewed image with that value false, update the existing separate Google Calendar consent configuration for the exact events scope, then enable it in a configuration-only deployment of the same digest. Disabling this flag rolls back creation without interrupting Calendar reads.
