...
Chore Management Increment 2 adds Consumption scheduled job `family-dashboard-staging-chore-generator`. It runs the same immutable backend image hourly at minute 7 with `--generate-chore-assignments`, connects privately to PostgreSQL, and receives no OAuth secret, PIN pepper, or Data Protection key permissions. Apply this structural Bicep change after the additive migration is available; subsequent releases reconcile its image automatically.
