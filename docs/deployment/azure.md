...
Chore Management Increment 2 adds Consumption scheduled job `family-dashboard-staging-chore`. It runs the same immutable backend image hourly at minute 7 with `--generate-chore-assignments`, connects privately to PostgreSQL, and receives no OAuth secret, PIN pepper, or Data Protection key permissions. The shortened Azure resource name remains within the 32-character Container Apps Job limit. Apply this structural Bicep change after the additive migration is available; subsequent releases reconcile its image automatically.
