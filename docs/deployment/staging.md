...
- The initial staging load exposed an authorization mismatch: the frontend requested the administration-protected household settings resource to obtain display preferences, so a non-elevated or locked shared-display session failed before loading events. The correction adds a Calendar-scoped, active-member-readable display-settings resource containing only time zone, locale, and week start; it preserves the administrative endpoint's existing authorization boundary and adds locked-display and cross-household regression coverage.
...
- Public tag `sha-94dc977` resolves to reviewed multi-architecture digest `sha256:7921d818c586d7a1e9d18dcedd6ce93d39c6f7109799139b50821e48a4d4e61f`. Migration execution `family-dashboard-staging-mig-faqb5ag` succeeded, and healthy/provisioned revision `family-dashboard-staging-api--0000043` runs that exact digest with 100% traffic.
- PostgreSQL Flexible Server `family-dashboard-staging-pg-rwzkcdch6czlm` is Ready on version 18 with public networking disabled. Public `/health/live` and database-backed `/health/ready` both return HTTP 200 `Healthy`.
- The existing locked-down storage account has public blob access and shared-key authorization disabled. Container `household-photos` reports `publicAccess: None`; the API runtime user-assigned identity has `Storage Blob Data Contributor`, and `HouseholdMedia__BlobContainerUri` is non-secret configuration. No storage credential or SAS is supplied to the API or frontend.
- `HouseholdMedia__Enabled=true`, `HouseholdMedia__Provider=AzureBlob`, `Weather__Enabled=true`, and `Weather__Provider=Nws` are reconciled. NWS requires no application credential.
- The public Netlify shell returns HTTP 200 and serves `/assets/index-DodKGfz8.js` with the approved `https://api.egobrane.net` origin and the personalization/weather interfaces. `/sw.js` returns `no-cache,max-age=0,must-revalidate`; Cloudflare reports `DYNAMIC`.
- The first staging upload exposed a multipart antiforgery metadata conflict: ASP.NET rejected the request before the application’s exact-origin/header antiforgery policy ran. Correction `94dc977` disables only framework form-antiforgery metadata for that endpoint while preserving `RequireFamilyDashboardAntiforgery`; a real-cookie PostgreSQL regression test covers the corrected boundary.
- After correction, the owner confirmed household photo upload, authenticated delivery, replacement, removal, focal positioning, custom/automatic greeting behavior, and fallback. Weather location configuration and forecast display remained operational.
- The owner completed the parent-PIN, household-isolation, responsive-device, and sensitive-data checks. Current repository, production-bundle, and short Azure-log scans found no storage credential, SAS token, private blob URL, OAuth token, PIN material, signing material, or other privileged value. Uploaded photos remain private personal data and the public demonstration image remains only a fallback.
- Dashboard Personalization Increment 2 is implemented locally but is not yet part of this deployment evidence. After the additive member-photo migration and matching frontend/backend deploy, owner verification must cover adult and child upload, replacement, removal, focal positioning, initials fallback, inactive-member history, attribution surfaces, private/shared-display authorization, household isolation, responsive devices, and sensitive-data inspection before this file records it as staging verified.
...
- The one-push GHCR publication, migration, and Azure API deployment succeeded. The first structural job-provisioning attempt failed closed during Azure provider preflight because the original job name exceeded 32 characters; no job was created. The corrected `family-dashboard-staging-chore` template then passed Azure preflight and provisioned successfully.
- The owner confirmed successful manual generator execution. Independent inspection on 2026-08-24 found successful automatic executions at minute 7. Daily/weekday generation, pause/resume, inactive dependency handling, time-zone and daylight-saving behavior, duplicate prevention, and responsive UI behavior remain recorded only as automated coverage unless the owner separately confirms those staging checks.

## Chore Management Increment 2 current deployment verification: 2026-08-24

- Repository `main` and GitHub both resolve to commit `4459a3f23c58c0d9279f1e57bafdd7ae75b48744`. [Continuous Integration run 32775229058](https://github.com/egobrane/FamilyBoard/actions/runs/32775229058) passed all frontend, backend, and container/manifests jobs. [Publish Backend Image run 32775229427](https://github.com/egobrane/FamilyBoard/actions/runs/32775229427) published successfully and its reusable staging deployment completed successfully.
- Public GHCR serves `ghcr.io/egobrane/familyboard-backend@sha256:5526933b2a876d778e619a29dc41a8d353ecd6d3163374c67b5f32168082ae3a`. Automatic digest handoff deployed that exact immutable image without a repository digest update.
- Migration execution `family-dashboard-staging-mig-t6o6bik` succeeded before API reconciliation. Azure API revision `family-dashboard-staging-api--0000028` is healthy, ready, scaled to zero when idle, and receives 100% of traffic on the reviewed digest.
- PostgreSQL Flexible Server `family-dashboard-staging-pg-rwzkcdch6czlm` is `Ready` on version 18 in Central US, public access is disabled, and UTF-8 database `family_dashboard` exists. Public liveness and database-backed readiness return HTTP 200 `Healthy`; unauthenticated `/api/auth/me` fails closed with HTTP 401 ProblemDetails code `authentication_required`.
- Scheduled Container Apps Job `family-dashboard-staging-chore` is provisioned successfully with schedule `7 * * * *`, command `--generate-chore-assignments`, the reviewed backend digest, and a `postgres-connection` secret reference. Recent successful executions include `family-dashboard-staging-chore-29793487`, `family-dashboard-staging-chore-29793547`, `family-dashboard-staging-chore-29793607`, and `family-dashboard-staging-chore-29793667`; the owner also confirmed a successful manual execution.
- `https://family.egobrane.net` returns the current PWA shell. Its compiled bundle references `https://api.egobrane.net` and includes the recurring-schedule interface; its manifest and service worker are available. Exact private Netlify deploy metadata was not available during this public verification.

## Chore Management Increment 3 local validation: 2026-08-25

- `AddChorePointLedger` is an additive, preservation-tested migration. It backfills assignment and completion point snapshots without awarding previously approved completions, and adds household-safe ledger actor/reversal constraints and indexes.
- The backend passed 102 tests with no skips against disposable local PostgreSQL 18, including empty and populated migration paths, atomic/concurrent single-award behavior, zero-point approval, rejection/retry, recurring snapshots, inactive-member history, adjustments, exact reversals, household isolation, and shared-display parent-elevation enforcement.
- Frontend lint, 30 component tests, the production PWA build, and 26 Playwright tests across wall-display and phone projects passed. The dashboard and `/points` now use API-backed balances/history; the adult correction UI creates immutable adjustments and explicit compensating reversals.
- Docker Compose validation, both production image builds, Azure Bicep and staging-parameter compilation, and both K3s renders passed locally. No Azure resource or new secret is required.

## Chore Management Increment 3 staging verification: 2026-08-25

- At the start of this verification, commit `7107a784b1471754dce2e9caa9f642431d7aee52` was cleanly checked out on `main` and matched `origin/main`. [Continuous Integration run 32852232777](https://github.com/egobrane/FamilyBoard/actions/runs/32852232777) and [Publish Backend Image run 32852234387](https://github.com/egobrane/FamilyBoard/actions/runs/32852234387) completed successfully. The latter automatically invoked the reusable staging workflow without a digest commit or second push.
- Public GHCR tag `sha-7107a78` resolves to multi-architecture digest `sha256:b7543874d6a07f6355c309043419672cf9557a002dfb560b19dedff7c0bd6424` for `linux/amd64` and `linux/arm64`, with provenance attestations.
- Migration execution `family-dashboard-staging-mig-n77m6k1` succeeded before API reconciliation. Retained Log Analytics records `Applying migration '20260825122424_AddChorePointLedger'` and its EF Core `10.0.10` migration-history insertion.
- Azure API revision `family-dashboard-staging-api--0000029` is active, Healthy, Provisioned, and receives 100% traffic on the reviewed digest. PostgreSQL Flexible Server `family-dashboard-staging-pg-rwzkcdch6czlm` is Ready on PostgreSQL 18 with public access disabled; UTF-8 database `family_dashboard` exists. Public liveness/readiness return `Healthy`, and unauthenticated `/api/auth/me` fails closed with HTTP 401 code `authentication_required`.
- Scheduled job `family-dashboard-staging-chore` uses the same digest, schedule `7 * * * *`, argument `--generate-chore-assignments`, and backend-only `postgres-connection` secret reference. Latest inspected execution `family-dashboard-staging-chore-ednksf3` succeeded.
- Netlify production deploy `6a8d95640345430008f15cea` is `ready` for commit `7107a784b1471754dce2e9caa9f642431d7aee52`. Public bundle `/assets/index-DqbpakHF.js` contains `https://api.egobrane.net` and the Increment 3 Points interfaces; the manifest and service worker are available. Cloudflare still assigns `/sw.js` a four-hour edge cache policy, so the previously documented cache-rule correction remains outstanding.
- The owner confirmed recurring chore operation, editing a definition to provide points to new work, the resulting point award/balance/history, and administrative point removal through the append-only correction workflow. Existing approved completions were not awarded retroactively by design. Zero-point approval, concurrent duplicate prevention, inactive-member history, shared-display parent-PIN enforcement, household isolation, and responsive-device behavior remain recorded as automated coverage unless separately owner-confirmed.

## Reward Management Increment 1 staging verification: 2026-08-25

- Reward UI commit `7db26b197fe16b9c701acef82ff0bf91bb31844c` deployed through Netlify production deploy `6a8dd663285b7d00087e4d79`. Its current bundle `/assets/index-DebLiKFB.js` contains `https://api.egobrane.net`, reward management, and redemption controls.
- The additive `AddRewardRedemptionWorkflow` migration succeeded through execution `family-dashboard-staging-mig-ljztzuk` before reward revision `family-dashboard-staging-api--0000030` received traffic. The owner then verified reward definition creation, editing, activation/deactivation, titles, descriptions, and point prices.
- The first authorized catalog request exposed an EF Core translation defect: filtering `IsActive` after constructing a member-balance DTO could not be translated. Azure application logs identified the query without exposing credentials or privileged request values. Definition administration remained functional because it used a separate endpoint.
- Correction commit `3ac3d23912a254054fa51eedb743f24df6e53966` added an authorized PostgreSQL catalog regression test and rewrote catalog reads as separately translated active-member and grouped-balance queries followed by in-memory response mapping. The current complete backend suite passes 107 tests with PostgreSQL enabled and no skips.
- [Continuous Integration run 32889016638](https://github.com/egobrane/FamilyBoard/actions/runs/32889016638) and [backend publication/deployment run 32889016692](https://github.com/egobrane/FamilyBoard/actions/runs/32889016692) succeeded for the correction commit. Public tag `sha-3ac3d23` resolves to multi-architecture OCI digest `sha256:4974160e513e9406e18a9836dff32daf15866b7d63ab7534d415b4bc942130b9` with attestations.
- Migration execution `family-dashboard-staging-mig-9oznksx` succeeded with the correction digest before API revision `family-dashboard-staging-api--0000031` became latest-ready and received 100% traffic. Public liveness and database-backed readiness return HTTP 200 `Healthy`.
- PostgreSQL Flexible Server `family-dashboard-staging-pg-rwzkcdch6czlm` is `Ready` on PostgreSQL 18 with public network access disabled; UTF-8 application database `family_dashboard` exists. Scheduled job `family-dashboard-staging-chore` uses the same reviewed digest, schedule `7 * * * *`, argument `--generate-chore-assignments`, and backend-only `postgres-connection` secret reference. Execution `family-dashboard-staging-chore-29794807` succeeded on that digest.
- The owner verified dashboard and full catalog display, private-session and shared-display attribution, insufficient-balance rejection, atomic point reservation, adult approval/rejection/cancellation/fulfillment, exact append-only release, point-cost snapshots, retained history, shared-display parent-PIN enforcement, household isolation, and touch, mouse, keyboard, screen-reader, phone, tablet, and wall-display behavior.
- Owner inspection found no OAuth token, client secret, database credential, PIN value, hash, pepper, signing material, or other privileged configuration in frontend assets, browser storage, URLs, source control, or logs.
- Cloudflare still returns `cache-control: max-age=14400, must-revalidate` for `https://family.egobrane.net/sw.js`, while direct Netlify returns `no-cache,max-age=0,must-revalidate`. The edge-cache correction and two-version physical Safari/wall-display proof remain open. A real Netlify Deploy Preview and an application rollback rehearsal also remain open.
## Google Calendar Increment 3 staging verification: 2026-08-26

- Commit `3343013bae1cb232092b8488c49f8d70f46ede32` was cleanly checked out on `main`. [Continuous Integration run 32983605393](https://github.com/egobrane/FamilyBoard/actions/runs/32983605393) and [backend publication/deployment run 32983605990](https://github.com/egobrane/FamilyBoard/actions/runs/32983605990) completed successfully, including frontend, backend, container/manifest, publication, and reusable protected staging jobs.
- Public GHCR tag `sha-3343013` resolves to multi-architecture OCI digest `sha256:93ac0319cc0f870c2d12059dd343bbeae7b5b069765342258ae0430570d2dfcf` for `linux/amd64` and `linux/arm64`, with attestations. Automatic immutable-digest handoff used that same digest without a second commit or manual digest entry.
- Migration execution `family-dashboard-staging-mig-qg1bzhk` succeeded before API reconciliation. This release includes additive migration `20260826141955_AddGoogleCalendarEventManagement`; owner-observed event-management operation and the successful database-backed readiness check corroborate the migrated schema without exposing the private connection string.
- Azure API revision `family-dashboard-staging-api--0000032` is latest-ready and Healthy, receives 100% traffic, and uses the reviewed digest. Approved frontend/CORS, Google sign-in, parent access, Calendar read/create/manage, callback, Data Protection, and chore-generation settings match the reviewed staging parameters; secrets appear only as named backend references.
- PostgreSQL Flexible Server `family-dashboard-staging-pg-rwzkcdch6czlm` is `Ready` on PostgreSQL 18 with public network access disabled; UTF-8 application database `family_dashboard` exists. Public liveness and database-backed readiness return HTTP 200 `Healthy`, while unauthenticated `/api/auth/me` fails closed with HTTP 401 code `authentication_required`.
- Scheduled job `family-dashboard-staging-chore` uses the same digest, schedule `7 * * * *`, argument `--generate-chore-assignments`, and backend-only `postgres-connection` secret reference. Its five latest inspected hourly executions succeeded; the latest was `family-dashboard-staging-chore-29795947`.
- Netlify production deploy `6a8eff2ae60931000898d742` is `ready` for the same commit. Public bundle `/assets/index-D2ZJAxw2.js` contains `https://api.egobrane.net`, `Manage event`, and the explicit Google Calendar deletion interface. The PWA manifest and service worker are available.
- The owner confirmed eligible simple Family Dashboard-created events expose management, editing changes the Google event, and explicit deletion removes it from Google Calendar. Google remains the sole event source of truth and PostgreSQL stores only non-content receipts. Externally created, recurring, and otherwise ineligible events remain read-only.
- The owner also confirmed stale-version/provider conflicts fail safely, repeated submissions recover idempotently, shared-display mutation requires parent-PIN elevation, household isolation holds, and touch, mouse, keyboard, screen reader, phone, tablet, and wall-display behavior succeeds. Inspection found no OAuth code/token, client secret, event content, database credential, PIN material, pepper, signing material, or privileged configuration in browser storage, URLs, frontend assets, source control, or logs.
- Cloudflare still returns `cache-control: max-age=14400, must-revalidate` for `https://family.egobrane.net/sw.js`, while direct Netlify returns `no-cache,max-age=0,must-revalidate`. The Cloudflare correction and two-version physical PWA proof remain open. A real safe Netlify Deploy Preview and an application rollback rehearsal also remain open.
