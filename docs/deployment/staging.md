...
## Google Calendar Increment 3 release candidate: 2026-08-26

The implementation adds the migration-first `AddGoogleCalendarEventManagement` release, the default-off `GoogleCalendar__EventManagementEnabled` gate, append-only mutation receipts, ETag-protected editing/deletion for simple Family Dashboard-created events, and the matching Netlify UI. It adds no OAuth scope, secret, dependency, or Azure resource. Deployment and owner staging proof remain pending; do not record the increment as staging verified until migration, immutable digest reconciliation, eligible/external/recurring boundaries, update, explicit delete, stale-version conflict, replay, shared-display PIN, multi-household isolation, responsive input, and leakage checks actually succeed.
