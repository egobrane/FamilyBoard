...
- Docker Compose validation, both production image builds, Azure Bicep and staging-parameter compilation, and both K3s renders passed locally. CI and staging deployment remain release steps. No Azure resource or new secret is required. The currently deployed staging revision remains Increment 2 until the approved change is pushed and the automatic migration/deployment workflow succeeds.
