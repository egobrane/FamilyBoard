...
Dashboard Personalization and Weather Increment 1 passes 121 backend tests against PostgreSQL 18 with no skips, 39 frontend component tests, and all 38 wall-display/phone Playwright scenarios. Coverage includes typed persistence, validation, authorized settings persistence, household isolation, NWS normalization and bounded retry, personalized dashboard copy, weather dialog accessibility, and private Azure/Compose/K3s storage rendering. Owner staging checks must cover real-file signature validation, metadata stripping, upload/replacement/removal, parent-PIN enforcement, NWS fresh/stale/unavailable behavior, dialog focus/Escape/backdrop behavior, and physical wall-display image framing.
